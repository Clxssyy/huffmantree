/*
    ...
    Implementation file for the Huffman struct
*/
...
void Huffman::create_codes(HNode* node, const std::string& code) {
    ...
    // if the node exists and isn't an internal node, add the character and it's code to the map
    if (node->value != '*') {
        codes[node->value] = code;
    }

    // recursively add to the code based on the way you go
...
void Huffman::serialize_tree(HNode* node, std::string& string) {
    ...
    // recursively go to the nodes children and add to the string
    serialize_tree(node->left, string);
    serialize_tree(node->right, string);
}
...
void Huffman::encode_string(const std::string& input, std::string& encoded_string) {
    ...
    // clear the encoded string to start fresh
    encoded_string.clear();

    // iterate over each character in the input string
    // adding the corresponding code for that character to the encoded string
    for (char c : input) {
        encoded_string += codes.at(c);
    }
}

Encoded Huffman::encode(const std::string& string) {
    ...
    // store it within ret
    encode_string(string, ret.encoded_string);
...
void Huffman::create_freq(const std::string& string) {
    ...
    for (int i = 0; i < string.size(); ++i) {
        ...
        if(freq.insert(std::make_pair(string[i], 1)).second == false) {
            auto iter = freq.find(string[i]);
            iter->second += 1;
        }
    }
}